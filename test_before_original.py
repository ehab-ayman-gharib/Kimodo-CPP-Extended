...
# Load keyframes into SOMA Armature
...
for mix_name, soma_name in BONE_MAPPING.items():
    if mix_name in char_arm.pose.bones and soma_name in soma_arm.pose.bones:
        pb = char_arm.pose.bones[mix_name]
        con = pb.constraints.new(type='COPY_ROTATION')
        con.target = soma_arm
        con.subtarget = soma_name
        con.mix_mode = 'BEFORE_ORIGINAL'
        con.target_space = 'LOCAL'
        con.owner_space = 'LOCAL'

print("Configured Mixamo constraints with mix_mode='BEFORE_ORIGINAL'")

# Check frame 0 vs frame 30
bpy.context.scene.frame_set(0)
bpy.context.view_layer.update()
print("Mixamo LeftArm rotation at f=0:", char_arm.pose.bones['mixamorig:LeftArm'].matrix.to_quaternion())
print("Mixamo LeftUpLeg rotation at f=0:", char_arm.pose.bones['mixamorig:LeftUpLeg'].matrix.to_quaternion())

bpy.context.scene.frame_set(30)
bpy.context.view_layer.update()
print("Mixamo LeftArm rotation at f=30:", char_arm.pose.bones['mixamorig:LeftArm'].matrix.to_quaternion())
print("Mixamo LeftUpLeg rotation at f=30:", char_arm.pose.bones['mixamorig:LeftUpLeg'].matrix.to_quaternion())
