...
num_frames = len(rot_data) // (len(SOMA30_NAMES) * 4)
rot_data = rot_data.reshape((num_frames, len(SOMA30_NAMES), 4))

print("\n--- ROTATIONS FOR LEFT SHOULDER & LEFT ARM ---")
for f in [0, 30]:
    q_sh = mathutils.Quaternion((rot_data[f, 10, 3], rot_data[f, 10, 0], rot_data[f, 10, 1], rot_data[f, 10, 2]))
    q_arm = mathutils.Quaternion((rot_data[f, 11, 3], rot_data[f, 11, 0], rot_data[f, 11, 1], rot_data[f, 11, 2]))
    print(f"Frame {f}:")
    print(f"  LeftShoulder delta: {q_sh @ mathutils.Quaternion((rot_data[0, 10, 3], rot_data[0, 10, 0], rot_data[0, 10, 1], rot_data[0, 10, 2])).inverted()}")
    print(f"  LeftArm delta: {q_arm @ mathutils.Quaternion((rot_data[0, 11, 3], rot_data[0, 11, 0], rot_data[0, 11, 1], rot_data[0, 11, 2])).inverted()}")
