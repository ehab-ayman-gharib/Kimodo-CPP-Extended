...
output_glb = r"E:\Kimodo-CPP\demo-output\5e15f54638196203\Remy_child_tail_test.glb"
...
# 2. Build SOMA Armature with child-pointing edit bones
...
# Compute absolute rest positions
...
# Primary child for each bone to define tail vector
PRIMARY_CHILD = {}
...
for i, name in enumerate(SOMA30_NAMES):
    ...
    # Point tail to actual child joint position
    if i in PRIMARY_CHILD:
        child_idx = PRIMARY_CHILD[i]
        eb.tail = abs_pos[child_idx]
    else:
        # For leaf bones (head, toes, hands), extend along parent direction
        p = SOMA30_PARENTS[i]
        if p >= 0:
...
for f in range(num_frames):
    ...
    # Forward direction in glTF: negate dz so character travels in its facing direction
...
# 3. Retarget SOMA -> Character Armature
bpy.context.view_layer.objects.active = char_arm
bpy.ops.object.mode_set(mode='POSE')
